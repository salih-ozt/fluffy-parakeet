...
async function authenticateApiKey(req, res, next) {
    ...
    try {
        const keyRow = await dbGet(
            `SELECT dak.*, u.id AS "ownerId" FROM dev_api_keys dak
        ...
        // Kullanım sayacını arttır (fire-and-forget)
        dbRun(
...
app.post('/api/dev/apikeys', authenticateToken, devKeyCreateLimiter, async (req, res) => {
    ...
    try {
        // Kullanıcı başına maks 10 anahtar (Free plan)
        const countRow = await dbGet(
            `SELECT COUNT(*) AS c FROM dev_api_keys WHERE "userId" = $1`,
            [req.user.id]
        );
        if (parseInt(countRow?.c || 0) >= 10) {
            return res.status(429).json({ error: 'Free plan ile en fazla 10 anahtar oluşturabilirsiniz' });
        }
